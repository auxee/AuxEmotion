            ...
            if (isOpenConfig)
            {
...
#if DEBUG
                ImGui.Begin("[DEBUG] AuxEmotion Configuration Window##AuxEmotionConfigWin", ref isOpenConfig, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse);
#else
                ImGui.Begin("AuxEmotion Configuration Window##AuxEmotionConfigWin", ref isOpenConfig, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse);
#endif
                if (ImGui.BeginChildFrame(ImGui.GetID("MainChildFrame"), new Vector2(ImGui.GetContentRegionAvail().X, ImGui.GetContentRegionAvail().Y - (50 * ImGuiHelpers.GlobalScale)), ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoBackground))
                {
                    ...
                    if (ImGui.BeginTabItem("Settings##TabSettings"))
                    {
                        ...
                        ImGui.SameLine();
                        ImGuiEx.InfoMarker($"Quick disable command:{Environment.NewLine}/auxe disable");

                        ImGui.Checkbox("Enable Chat Reaction##isChatActive", ref configGUI.isChatActive);
                        ImGui.SameLine();
                        ImGuiEx.InfoMarker("No dedicated slash command for this toggle.");
                        ...
                        ImGui.SameLine();
                        ImGuiEx.InfoMarker("When enabled the emotes used by AuxEmotion will not display the log message.");
